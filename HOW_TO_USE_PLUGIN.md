...
- **🖱️ 自由拖拽**：点击卡片标题栏可以拖拽移动到画布任意位置
- **📏 智能边界**：卡片移动时自动限制在可视区域内，不会超出边界
- **🔝 自动置顶**：点击任意卡片会自动将其提升到最上层
- **➖ 最小化**：点击 "−" 按钮折叠卡片内容，只保留标题栏
- **⬜ 最大化**：点击 "⬜" 按钮将卡片全屏显示
- **❌ 关闭卡片**：点击 "×" 按钮带动画效果关闭卡片
- **📑 多卡片管理**：可以同时显示多个卡片，每个都可以独立操作

### 🎨 视觉效果增强
- **背景模糊**：卡片采用毛玻璃效果，美观且不干扰背景内容
- **悬停反馈**：鼠标悬停时卡片会轻微上浮，提供视觉反馈
- **拖拽动画**：拖拽时卡片会轻微旋转和缩放，增强拖拽感受
- **平滑过渡**：所有状态变化都有平滑的过渡动画
- **层级管理**：多卡片时智能管理显示层级，避免遮挡

### 📱 响应式设计
- **移动友好**：在小屏幕上自动调整卡片大小和布局
- **全屏适配**：最大化时完美适配不同屏幕尺寸
- **触摸支持**：支持触摸设备的拖拽操作
